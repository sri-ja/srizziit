...
const Bookmarks = () => {
	...
	useEffect(() => {
		if (localStorage.getItem("jwtToken") === null) navigate("/");
			...
			axios.get("http://localhost:5000/api/post/bookmarks", { params: { token: token } }).then((res) => {
				if (res.data.error) {
					setError(res.data.error);
					window.location.reload();
				} else {
    ...
    return (
        <div>
			...
			<Modal open={newReport.reporter?.length ? true : false} onClose={() => closeModal()}>
				<Box sx={style}>
					...
					<form className="new-post">
						...
						<Button sx={{ marginTop: "12px" }} variant="contained" size="large" color="secondary" onClick={(e) => createReport(e)}>
							report
						</Button>
					</form>
				</Box>
            ...
            <div style={{paddingTop: "30px"}} className="center-align">
                <Container maxWidth="lg">
                <h3 style={{paddingBottom: "20px"}}>posts bookmarked by you</h3>

				{posts?.map((post) => (
					<div className="row" key={post?._id}>
						<div className="col m12">
							<div className="sub-card w-100 left-align">
								<div className="card-content">
									<p style={{ fontSize: "25px"}}>{post?.text}</p>
								</div>
								<div className="card-action">
									<p style={{ color: "purple", fontStyle: "italic", fontSize: "18px" }}>posted in <NavLink to={"/sub/" + post?.sub}><span style={{fontWeight: "700", color: "purple"}}>{post?.sub}</span></NavLink> by <span style={{fontWeight: "700"}}>{post?.posted_by}</span></p>
